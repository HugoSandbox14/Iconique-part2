...
def get_sub_df(df,sub):
    sub_df = df[df['subjects'] == sub]      # selection des meta data relative a sub
    sub_df.index = np.arange(0,len(sub_df)) # reindexage
    return sub_df

def get_tmax(df,std_bad_epoch):
    mean = df['latency'].mean()
    std = df['latency'].std()
    th_bad_epoch = mean + std_bad_epoch * std              # critere de selection pour definir ce qu'est un 'bad epoch' sur la base du temps de latence entre stimulation et reponse

    df_clean = df[df['latency'] < th_bad_epoch]
    tmax = df_clean['latency'].mean()
    return tmax 


def get_idx_bad_epoch(bad_epochs_auto):

    print(f"nb d'epoch = {len(bad_epochs_auto)}")
    idx_bad_epoch = []
    for i, el in enumerate(bad_epochs_auto):
        if el == True:
            idx_bad_epoch.append(i)
    print(f"nb de bad epoch = {len(idx_bad_epoch)}")
    return idx_bad_epoch

def get_mean_interpolate_ch(idx_bad_epoch,bad_ch_auto):
    nb_bad_ch_per_epoch = []
    for i,epoch in enumerate(bad_ch_auto):
        nb_bad_ch = 0
        if i in idx_bad_epoch :
            pass
        else :
            for ch in epoch:
                if ch == 1:
                    nb_bad_ch+=1
                nb_bad_ch_per_epoch.append(nb_bad_ch)
    mean_interpoplate_ch = np.array(nb_bad_ch_per_epoch).mean()        
    print(f"nb moyen de ch interpolee = {mean_interpoplate_ch}")
    return mean_interpoplate_ch
